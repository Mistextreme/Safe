...
lib.callback.register('ox_inventory:renameItem', function(source, slot, label)
	local inventory = Inventory(source)

	if label == "" or string.match(label, "^%s*$") or #label <= 0 then
		return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = "Veuillez entrer un nom qui n'est pas vide." })
	end
	if string.match(label, "\\x") then
		return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = "Veuillez entrer un nom valide." })
	end

	if #label > 60 then 
		return TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = "Veuillez entrer un nom qui ne dépasse pas 60 caractères." })
	end

	if inventory.player then
		local item = inventory.items[slot]
		if item then
			local data = item.metadata
			if not data then data = {} end
			data.label = label
			inventory.items[slot].metadata = data
			Inventory.SetMetadata(source, slot, data)
			print(json.encode(inventory.items[slot].metadata, { indent = true }))
...
exports('GetWeightItems', function(nameItems, count)
	local weight = 0
	local item = Items(nameItems)
	if item then
