...
function client.openInventory(inv, data)
	...
	if not right or not right.id then 
	end
	exports['NoPEDOX']:CreateVirtualPed()
    return true
end
...
function client.closeInventory(server)
	...
	if invOpen then
		...
		closeTrunk()
		exports['NoPEDOX']:DestroyVirtualPed()
...
-------------------------------------------------
-- LOGIQUE POUR LE CHANGEMENT DE COIFFURE
-------------------------------------------------
-- Remplacez les valeurs par défaut et alternatives selon vos besoins
local originalHairStyle = { hairstyle = 0, hairColor = 0 }       -- valeurs par défaut de votre personnage
local alternativeHairStyle = { hairstyle = 1, hairColor = 1 }      -- valeurs de la coiffure alternative
local isOriginalHair = true

RegisterNUICallback('setHair', function(data, cb)
    local playerPed = PlayerPedId()
    if isOriginalHair then
        -- Sauvegarder la coiffure actuelle avant de basculer
        originalHairStyle.hairstyle = GetPedDrawableVariation(playerPed, 2)
        originalHairStyle.hairColor = GetPedHairColor(playerPed)
        
        -- Appliquer la coiffure alternative
        SetPedComponentVariation(playerPed, 2, alternativeHairStyle.hairstyle, 0, 0)
        SetPedHairColor(playerPed, alternativeHairStyle.hairColor, alternativeHairStyle.hairColor)
        isOriginalHair = false
    else
        -- Revenir à la coiffure d'origine
        SetPedComponentVariation(playerPed, 2, originalHairStyle.hairstyle, 0, 0)
        SetPedHairColor(playerPed, originalHairStyle.hairColor, originalHairStyle.hairColor)
        isOriginalHair = true
    end

    exports['NoPEDOX']:UpdatePed()
    cb('ok')
end)
